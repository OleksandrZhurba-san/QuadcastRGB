...
#define BUSY_ERR_MSG                                                           \
  _("Another program is using the microphone already. "                        \
    "Stopping.\n")
#define TRANSFER_ERR_MSG                                                       \
  _("Couldn't transfer a packet! "                                             \
...
/* Error codes */
enum { libusberr = 2, nodeverr, devopenerr, transfererr };
...
/* Product IDs */
const unsigned short product_ids_kingston[] = {0x171f};
const unsigned short product_ids_hp[] = {
    0x0f8b, 0x028c, 0x048c, 0x068c, 0x098c /* Duocast */
};
...
static int _claim_dev_interface(libusb_device_handle *handle) {
  int errcode0, errcode1;
...
static void display_data_arr(libusb_device_handle *handle,
  ...
  short sent;
  byte_t *packet;
  byte_t header_packet[PACKET_SIZE] = {HEADER_CODE, DISPLAY_CODE,
                                       0,           0,
                                       0,           0,
                                       0,           0,
                                       PACKET_CNT,  0,
  ...
  while (colcommand < end && nonstop) {
    ...
    if (sent != PACKET_SIZE) {
      nonstop = 0;
      break; /* finish program in case of any errors */
    }
    memcpy(packet, colcommand, 2 * BYTE_STEP);
    sent =
        libusb_control_transfer(handle, BMREQUEST_TYPE_OUT, BREQUEST_OUT,
