...
static void
fill_data(struct colscheme *colsch, byte_t *da, int pckcnt, int group);
...
static void
sequence_blink(const struct colscheme *colsch, byte_t *da, int pckcnt);
...
static void         sequence_lightning(const int *color,
                                       int        spd,
                                       int        group,
                                       int        synchronous,
                                       byte_t    *da);
...
short count_color_commands(const datpack *data_arr, int pck_cnt, int colgroup)
{
    ...
    if (colgroup) /* case of lower color commands */
        step = BYTE_STEP;
    cnt = (pck_cnt - 1) * 8;
    for (b = data_arr[pck_cnt - 1];
         b < data_arr[pck_cnt - 1] + DATA_PACKET_SIZE; b += 2 * BYTE_STEP)
    {
        if (*(b + step) != RGB_CODE)
            break;
        cnt++;
...
static int count_data(struct colscheme *colsch)
{
    ...
    else if (strequ(colsch->mode, "lightning") || strequ(colsch->mode, "pulse"))
    {
...
static int get_gradient_length(const int *color, int spd)
{
    ...
    if (tr_size * color_cnt > MAX_COLPAIR_COUNT)
        return MIN_CYCL_TR + (MAX_COLPAIR_COUNT / color_cnt - MIN_CYCL_TR) *
                                 (100 - spd) / 100;
    return tr_size;
}
        ...
        **da = RGB_CODE;
        ...
        for (j = 0; j < 3; j++, (*da)++)
        {
            **da = rgb_curr[j]; /* write R, G, or B */
            ...
            rgb_curr[j] = (int)(rgb_st[j] + ((float)(i) / (length - 1)) *
                                                (rgb_end[j] - rgb_st[j]));
        }
    }
}
...
static int next_gradient_color(int color, int endcolor, unsigned int size)
{
    ...
    for (shift = 16, i = 0; shift >= 0; shift -= 8, i++)
    {
        ...
        /* Perform one step */
        rgb[i] =
            (int)(rgb[i] + ((float)(1) / (size - 1)) * (rgb_end[i] - rgb[i]));
