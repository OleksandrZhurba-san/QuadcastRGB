...
/* Static declarations */
static void set_arg(const char      ***arg_pp,
                    const char       **argv_end,
                    struct colschemes *cs,
                    int               *state,
                    int               *verbose);
static void set_br_spd_dly(const char       **arg_p,
                           const char       **argv_end,
                           int                state,
                           struct colschemes *cs);
static void set_mode(const char      ***arg_pp,
                     const char       **argv_end,
                     int                state,
                     struct colschemes *cs);
static void set_colors(const char      ***arg_pp,
                       const char       **argv_end,
                       int                state,
                       struct colschemes *cs);
...
/* Const arrays */
const char      *modes[MODES_CNT] = {"solid",     "blink", "cycle",     "wave",
                                     "lightning", "pulse", "visualizer"};
static const int rainbow[RAINBOW_CNT] = {0xff0000, 0xff009e, 0xcd00ff, 0x2b00ff,
                                         0x0068ff, 0x00ffff, 0x00ff67, 0x32ff00,
                                         0xceff00, nocolor};
...
struct colschemes *parse_arg(int argc, const char **argv, int *verbose)
{
    ...
    if (!(cs->upper.mode))
    { /* any chosen group sets also the other */
                     ...
                     struct colschemes *cs)
{
    ...
    if (!(cs->upper.mode) || !(cs->lower.mode))
    { /* write solid to the other */
...
static void write_default_cols(struct colschemes *cs, int state)
{
    ...
    else if (strequ(md, modes[1]))
    { /* blink */
        write_int_param(cs->upper.colors, cs->lower.colors, nocolor, state);
    }
    else
    { /* solid, lightning, pulse */
